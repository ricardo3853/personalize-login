 ...
 * Description:       Plugin que altera o fluxo de login, registro e esqueceu senha no wordpress.
 * Version:           2.1.0
...
class Personalize_Login_Plugin {
	...
	public function do_register_user() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( ! get_option( 'users_can_register' ) ) {
				...
				$role = $_POST['tipo_role'];

				$result = $this->register_user( $email, $first_name, $last_name, $role );
	 ...
	 */
	private function register_user( $email, $first_name, $last_name,$role ) {
		$errors = new WP_Error();
